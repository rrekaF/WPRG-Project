...
function logged_in() {
  $session = $_COOKIE["bussession"] ?? null;
  if (!$session) {
    return false;
  }
  ...
  <div class="container mx-auto size-full flex items-center justify-between px-8">
    ...
    <nav>
      <ul role="list" class="flex items-center gap-6">
        <?php if (logged_in()): ?>
          <li>
            <a href="account.php" class="block text-black hover:underline font-grotesk text-sm font-medium">Account</a>
          </li>
        <?php else: ?>
          <li>
            <a href="login.php" class="block text-black hover:underline font-grotesk text-sm font-medium">Log in</a>
          </li>
          <li>
            <a href="register.php" class="block bg-black text-white p-3 rounded-md font-grotesk text-sm font-medium transition-transform active:scale-95 hover:bg-black/80">Sign up</a>
          </li>
        <?php endif; ?>
      </ul>
    </nav>
